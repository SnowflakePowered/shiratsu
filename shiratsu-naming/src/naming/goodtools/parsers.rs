...
use nom::bytes::complete::{
    is_not, take_till1, take_until, take_while, take_while1, take_while_m_n,
};
...
pub(crate) fn parse_region(input: &str) -> IResult<&str, (Vec<&str>, Vec<Region>)> {
    let (strs, regions) =
        Region::try_from_goodtools_region_with_strs(input).map_err(|e| match e {
...
fn parse_version(input: &str) -> IResult<&str, GoodToolsToken> {
    ...
    // x.xx also possible.
    let (input, _) = peek(take_while1(|c: char| c.is_ascii_digit() || c == 'x'))(input)?;

    // Take all digits (or 'x') up to .
    let (input, major) = take_while1(
...
fn parse_dump_tag<'a>(
    infotag: &'static str,
) -> impl FnMut(&'a str) -> IResult<&'a str, GoodToolsToken<'a>> {
    move |input: &str| {
        let (input, _) = char('[')(input)?;
        let (input, itag) = tag(infotag)(input)?;
        let (input, number) = opt(take_while1(|c: char| {
...
pub(super) fn do_parse(input: &str) -> IResult<&str, Vec<GoodToolsToken>> {
    ...
    fn parse_all_tags_until_end(input: &str) -> IResult<&str, Vec<GoodToolsToken>> {
        let (input, tokens) = many1(preceded(
            opt(tag(" ")),
            alt((
                parse_known_tag,
    ...
    if let Ok((input, (title, mut tokens))) = take_up_to(parse_all_tags_until_end)(input) {
...
mod test {
    ...
    fn test_parse() {
        ...
        println!(
            "{:?}",
            do_parse("2 Pak Special (Magenta) - Cavern Blaster, City War (1992) (HES) (PAL) [!]")
                .unwrap()
        );
        println!(
            "{:?}",
            do_parse("Fire Emblem - Ankoku Ryuu to Hikari no Tsurugi (J) [hM04][b1]").unwrap()
        );
    }
    ...
    fn test_year() {
        assert_eq!(
            parse_year_tag("(1999)"),
            Ok(("", GoodToolsToken::Year("1999")))
        );
        assert_eq!(
            parse_year_tag("(19xx)"),
            Ok(("", GoodToolsToken::Year("19xx")))
        );
        assert_eq!(
            parse_year_tag("(20xx)"),
            Ok(("", GoodToolsToken::Year("20xx")))
        );
    }
}
