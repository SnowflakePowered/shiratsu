...
fn parse_dumpinfo_tag<'a>(
    infotag: &'static str,
) -> impl FnMut(&'a str) -> IResult<&'a str, TOSECToken<'a>> {
    move |input: &'a str| {
...
fn parse_devstatus(input: &str) -> IResult<&str, Vec<TOSECToken>> {
    ...
    .map(|c| {
        Ok(vec![
            TOSECToken::Warning(TOSECWarn::MalformedDevelopmentStatus(c)),
...
fn parse_date(input: &str) -> IResult<&str, Vec<TOSECToken>> {
    fn parse_undelimited_date(input: &str) -> IResult<&str, Vec<TOSECToken>> {
        ...
        let (input, month) = verify(
            take_while_m_n(2, 2, |c: char| c.is_ascii_digit()),
            |month: &str| month.parse::<u32>().map(|m| m <= 12).unwrap_or(false),
        )(input)?;

        let (input, day) = verify(
            take_while_m_n(2, 2, |c: char| c.is_ascii_digit()),
            |month: &str| month.parse::<u32>().map(|m| m <= 31).unwrap_or(false),
        )(input)?;
    ...
    let (input, month) = opt(preceded(
        char('-'),
        take_while_m_n(2, 2, |c: char| c.is_ascii_digit() || c == 'X' || c == 'x'),
    ))(input)?;
    let (input, day) = opt(preceded(
        char('-'),
        take_while_m_n(2, 2, |c: char| c.is_ascii_digit() || c == 'X' || c == 'x'),
    ))(input)?;

    if year.contains('X') {
        parses.push(TOSECToken::Warning(TOSECWarn::MalformedDatePlaceholder(
            year,
        )));
    }
...
fn parse_region(input: &str) -> IResult<&str, (Vec<&str>, Vec<Region>)> {
    let regions = Region::try_from_tosec_region_with_strs(input).map_err(|e| match e {
...
fn parse_publisher(input: &str) -> IResult<&str, TOSECToken> {
   ...
   fn parse_dash_publisher(input: &str) -> IResult<&str, &str> {
    ...
    let (input, publishers) =
        separated_list1(tag(" - "), alt((take_until_is(")", " - "), is_not(")"))))(input)?;
...
fn parse_language(input: &str) -> IResult<&str, TOSECToken> {
    ...
    fn parse_double_lang(input: &str) -> IResult<&str, TOSECToken> {
        ...
        Ok((
            input,
            TOSECToken::Languages(TOSECLanguage::Double(lang1, lang2)),
        ))
    }
...
fn parse_media(input: &str) -> IResult<&str, TOSECToken> {
    ...
    let (input, side) = opt(preceded(char(' '), parse_side))(input)?;
    parts.push(part);
...
fn parse_version_tag(input: &str) -> IResult<&str, Vec<TOSECToken>> {
    ...
    Ok((
        input,
        vec![TOSECToken::Warning(TOSECWarn::VersionInFlag), version],
    ))
}
...
fn parse_title_demo_date_happy(input: &str) -> IResult<&str, Vec<TOSECToken>> {
    fn parse_version_and_demo_or_date(input: &str) -> IResult<&str, Vec<TOSECToken>> {
        ...
        let (input, demo) = opt(preceded(opt(char(' ')), parse_demo_tag))(input)?;
    ...
    let (input, (title, mut tokens)) = take_up_to(parse_version_and_demo_or_date)(input)?;
...
fn parse_unexpected_space(input: &str) -> IResult<&str, char> {
    if peek::<&str, &str, nom::error::Error<&str>, _>(tag(" & "))(input).is_ok() {
        return Err(nom::Err::Error(nom::error::Error::from_error_kind(
            input,
            ErrorKind::Tag,
        )));
...
fn parse_title_degenerate_path(input: &str) -> IResult<&str, Vec<TOSECToken>> {
    ...
    let (input, (title, version)) = take_up_to(alt((
        preceded(char(' '), parse_version_string).map(Some),
...
fn parse_tosec_name(input: &str) -> IResult<&str, Vec<TOSECToken>> {
    ...
    fn parse_by_publisher(input: &str) -> IResult<&str, (TOSECToken, TOSECToken)> {
        ...
        Ok((
            input,
            (
                TOSECToken::Title(title.trim()),
                TOSECToken::Publisher(Some(vec![publisher.trim()])),
            ),
        ))
    }
        ...
        Some(zzz) => {
            ...
            let input = match tokens.first() {
                Some(&TOSECToken::Title(title)) => {
                    if let Ok((_, (title, TOSECToken::Publisher(publisher)))) =
                        parse_by_publisher(title)
                    {
                        tokens[0] = title; // replace title.
                        tokens.insert(1, TOSECToken::Publisher(publisher));

                        tokens.insert(1, TOSECToken::Warning(TOSECWarn::ByPublisher));
                        tokens.insert(1, TOSECToken::Warning(TOSECWarn::PublisherBeforeDate));
                        input
    ...
    .is_err()
    {
        ...
        if !tokens.iter().any(|c| matches!(c, TOSECToken::Publisher(_))) {
            tokens.push(TOSECToken::Warning(TOSECWarn::MissingPublisher));
        }
        input
    };
    ...
    let (input, rest) = many0(pair(opt(parse_unexpected_space), parse_moreinfo_tag))(input)?;
...
pub(crate) fn do_parse_multiset(
    input: &str,
) -> IResult<&str, (Vec<Vec<TOSECToken>>, Vec<TOSECToken>)> {
    let (input, tokens) = separated_list1(tag(" & "), parse_tosec_name)(input)?;
    ...
    let input = if let Ok((input, _)) = char::<&str, nom::error::Error<&str>>('-')(input) {
        let (input, flags) = many0(pair(
            opt(parse_unexpected_space),
            alt((parse_known_flags, map(parse_parens_tag, |t| vec![t]))),
        ))(input)?;
...
mod test {
    ...
    fn test_repair() {
        assert_eq!(
            do_parse("ZZZ-UNK-Loopz (U) (Beta) (v0.06)")
                .map(|(i, e)| (i, e.into()))
    ...
    fn test_zzz_unks() {
        assert_eq!(
            do_parse("ZZZ-UNK-Raiden (U) (CES Version) (v3.0)"),
            Ok((
                "",
                vec![
        ...
        assert_eq!(
            do_parse("ZZZ-UNK-Space Lock (Rev 20040529)(Beta)"),
            Ok((
                "",
                vec![
                    TOSECToken::Warning(TOSECWarn::ZZZUnknown),
        ...
        assert_eq!(
            do_parse("ZZZ-UNK-Tron 6 fun v0.15"),
            Ok((
                "",
                vec![
                    TOSECToken::Warning(TOSECWarn::ZZZUnknown),
    ...
    fn globals_to_string() {
        for string in &["Amidar (19xx)(Devstudio) & Amigos (1987)(Mr. Tosec)-(PD)(Disk 1 of 2)[a]"]
        {
            assert_eq!(
    ...
    fn test_ambiguous_version() {
        assert_eq!(
            do_parse("ZZZ-UNK-Alien vs Predator (U) (Beta)"),
            Ok((
                "",
                vec![
                    TOSECToken::Warning(TOSECWarn::ZZZUnknown),
    ...
    fn test_parse_full() {
        ...
        assert_eq!(
            do_parse(
                "Motocross & Pole Position Rev 1 (Starsoft - JVP)(PAL)[b1][possible unknown mode]"
            ),
            Ok((
                "",
                vec![
    ...
    fn test_parse_lang() {
        assert_eq!(
            parse_language_tag("(M6)"),
            Ok(("", TOSECToken::Languages(TOSECLanguage::Count("6"))))
        );
        assert_eq!(
            parse_language_tag("(M16)"),
            Ok(("", TOSECToken::Languages(TOSECLanguage::Count("16"))))
        );
        assert_eq!(
            parse_language_tag("(en)"),
            Ok(("", TOSECToken::Languages(TOSECLanguage::Single("en"))))
        );
        assert_eq!(
            parse_language_tag("(en-ja)"),
            Ok(("", TOSECToken::Languages(TOSECLanguage::Double("en", "ja"))))
        );
    }
    ...
    fn test_parse_region() {
        assert_eq!(
            parse_region("US"),
            Ok(("", (vec!["US"], vec![Region::UnitedStates])))
        );
        assert_eq!(
            parse_region("US-ZZ"),
            Ok((
    ...
    fn test_parse_publisher() {
        ...
        assert_eq!(
            parse_publisher_tag("(Devstudio)"),
            Ok(("", TOSECToken::Publisher(Some(vec!["Devstudio"]))))
        );
        assert_eq!(
            parse_publisher_tag("(Ultrafast Software)"),
            Ok(("", TOSECToken::Publisher(Some(vec!["Ultrafast Software"]))))
        );
        assert_eq!(
            parse_publisher_tag("(Smith, R. - White, P.S.)"),
            Ok((
                "",
        ...
        assert_eq!(
            parse_publisher("Smith, R. - White, P.S."),
            Ok((
                "",
                TOSECToken::Publisher(Some(vec!["Smith, R.", "White, P.S."]))
            ))
